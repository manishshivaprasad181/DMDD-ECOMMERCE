...
BEGIN
    ...
    WHERE oi.order_id = p_order_id AND ROWNUM = 1; -- Add this line to limit to one row
    ...
    FROM product
    WHERE product_id = v_product_id AND ROWNUM = 1; -- Add this line to limit to one row
    ...
    -- Update the existing record in order_item for the return
    UPDATE /*+ NO_PARALLEL(order_item) */ order_item
